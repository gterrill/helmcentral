...
export interface NotePatch {
  ...
  type?: NoteType
  body?: string
  /** Plan §8 (Mate): the operator-facing way to pin/unpin a note for Mate's
   * pinned-notes prompt. Doesn't touch the note's rendered frontmatter -
   * patchNoteHandler applies it as a metadata-only write. */
  pinned?: boolean
}
...
// upgrades the classifier, after notes already exist) - free-standing
// functions, not hook state, since both are one-shot reads/writes a single
// call site (documents-panel.tsx's toolbar) drives directly, the same way
// useDocuments.ts keeps dryRunEmbeddingsBackfill/startEmbeddingsBackfill as
// plain callbacks rather than owning their own polled status. ──────────

/** POST /api/notes/classify/backfill?dry_run=1 response shape
 * (notes_handlers.go) - starts nothing. */
export interface NotesClassifyBackfillDryRun {
  count: number
}

/** POST /api/notes/classify/backfill response shape - count is how many
 * notes actually changed classification, which can be fewer than the dry
 * run's count (a note whose classifier answer already matched isn't
 * touched). */
export interface NotesClassifyBackfillResult {
  count: number
}

/** POST /api/notes/enrich/backfill?dry_run=1 response shape - starts
 * nothing. tokens_estimate is chars_pending/4, the same rough scale-setting
 * figure DocumentEmbeddingsBackfillDryRun's own field is (use-documents.ts),
 * not a price quote. */
export interface NotesEnrichBackfillDryRun {
  count: number
  tokens_estimate: number
}

export interface NotesEnrichBackfillResult {
  count: number
}

/** Local classifier only - no network call, no OpenRouter spend, costs
 * nothing to run or to dry-run. */
export async function dryRunNotesClassifyBackfill() {
  return submitJSON<NotesClassifyBackfillDryRun>(`${apiBaseUrl}/api/notes/classify/backfill?dry_run=1`, 'POST')
}

export async function runNotesClassifyBackfill() {
  return submitJSON<NotesClassifyBackfillResult>(`${apiBaseUrl}/api/notes/classify/backfill`, 'POST')
}

export async function dryRunNotesEnrichBackfill() {
  return submitJSON<NotesEnrichBackfillDryRun>(`${apiBaseUrl}/api/notes/enrich/backfill?dry_run=1`, 'POST')
}

/** Making this call (no ?dry_run) IS the operator's consent for the
 * pending notes' text to reach OpenRouter - the same "the POST itself is
 * the consent" contract startEmbeddingsBackfill's own comment documents
 * (use-documents.ts). A 400 (Mate not configured/ready) still throws the
 * server's own message through submitJSON, unlike that sibling function's
 * special-cased 409 - there is no "already running" state here to fold in. */
export async function runNotesEnrichBackfill() {
  return submitJSON<NotesEnrichBackfillResult>(`${apiBaseUrl}/api/notes/enrich/backfill`, 'POST')
}
