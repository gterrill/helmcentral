...
// assistant-markdown-impl.tsx's h1-h4 renderers only ever destructure
// `children`, so passing them an `id` prop would silently do nothing - they'd
// have to forward it themselves, which would mean spreading `node` (a hast
// object) onto the DOM element too. Cheaper and clearer to own these four
// elements here, duplicating their four style strings, than to teach a
// chat-reply renderer about ids and hast internals it otherwise has no
// reason to know.
...
interface ManualMarkdownImplProps {
   ...
   * re-trigger the scroll once the real content mounts. */
  onRendered?: () => void
}
