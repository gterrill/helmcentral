...
type ScanEvent =
  ...
  | { id: string; kind: 'elsewhere'; item: EquipmentItem; recordedBinCode: string | null }
...
export function StocktakeSection() {
  ...
  const handleScan = async (rawText: string) => {
    ...
    if (parsed.panel === 'inventory' && parsed.inventorySection === 'equipment' && parsed.equipmentEditId) {
      ...
      pushEvent({ id: crypto.randomUUID(), kind: 'elsewhere', item, recordedBinCode: item.bin_code || null })
      return
    }
  ...
  const handleMove = async (eventId: string, item: EquipmentItem) => {
    ...
    try {
      ...
      await updateEquipment(item.id, { ...toEquipmentInput(item), bin_id: currentBin.bin.id, zone_id: currentBin.zone.id })
      setConfirmedIds((prev) => new Set(prev).add(item.id))
      setEvents((prev) => prev.map((e) => (e.id === eventId ? { id: e.id, kind: 'confirmed', item } : e)))
    } catch (err) {
      setScanError(err instanceof Error ? err.message : String(err))
    } finally {
      setMovingId(null)
              ...
              {event.kind === 'elsewhere' && (
                <>
                  ...
                  <Button
                    ...
                    onClick={() => { void handleMove(event.id, event.item) }}
                  >
                    {movingId === event.item.id ? 'Moving...' : `Move to ${currentBin?.bin.code ?? ''}`}
                  </Button>
                </>
              )}
