...
describe('ManualMarkdown heading ids', () => {
  it('gives an h2 (## in the markdown) the GitHub slug id', () => {
    render(<ManualMarkdown content="## The kiosk feed" pageId="features/dashboard" onNavigate={vi.fn()} />)

    const heading = screen.getByText('The kiosk feed')
  ...
  it('gives an h3 (### in the markdown) the GitHub slug id too', () => {
...
describe('ManualMarkdown links', () => {
  it('prevents the default navigation and calls onNavigate for an in-tree page link', () => {
    const onNavigate = vi.fn()
    render(
      <ManualMarkdown content="[Alarms](alarms.md)" pageId="features/dashboard" onNavigate={onNavigate} />,
    )

    const link = screen.getByRole('link', { name: 'Alarms' })
  ...
  it('treats a #hash link as an anchor and calls onNavigate with the anchor kind', () => {
    const onNavigate = vi.fn()
  ...
  it('keeps target=_blank and rel=noreferrer, and does not call onNavigate, for an external link', () => {
    const onNavigate = vi.fn()
    render(
      <ManualMarkdown
    ...
    const link = screen.getByRole('link', { name: 'wazero' })
...
describe('ManualMarkdown img handling', () => {
  it('drops an <img> tag from the rendered output', () => {
