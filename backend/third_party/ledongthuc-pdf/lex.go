...
Loop:
	for !b.eof {
		...
		case '\\':
			...
			default:
				// ISO 32000-2:2020 §7.3.4.2: "If the character following
				// the REVERSE SOLIDUS is not one of those shown [in the
				// escape-sequence table]... the reverse solidus shall be
				// ignored." So an unrecognised escape keeps only the
				// character itself, not the backslash, and is not an
				// error - PDF writers do emit this (for example escaping
				// an ordinary letter that needs no escaping at all).
				tmp = append(tmp, c)
			case 'n':
				tmp = append(tmp, '\n')
			case 'r':
			...
			case '0', '1', '2', '3', '4', '5', '6', '7':
				...
				// ISO 32000-2:2020 §7.3.4.2: an octal escape may be one,
				// two or three digits, and "high-order overflow shall be
				// ignored" - three octal digits can encode up to 0o777
				// (511), so this is an expected, spec-legal case, not a
				// malformed file. byte(x) below already keeps only the
				// low 8 bits, so overflow is silently and correctly
