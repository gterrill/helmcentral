...
func applyFilter(rd io.Reader, name string, param Value) io.Reader {
	...
	case "FlateDecode":
		...
		default:
			...
			panic("pred")
		// Predictor 1 means "no prediction" (ISO 32000-2:2020 §7.4.4.4,
		// Table 8): the FlateDecode output is the row data as-is, with no
		// per-row filtering layered on top of it. This is not the same as
		// the /DecodeParms dictionary (or /Predictor key) being absent
		// entirely - pred.Kind() == Null above already handles that case -
		// it is an explicit "1" that a number of real-world PDF writers do
		// emit, and treating it as an unknown predictor (the default case)
		// panicked on every one of them even though the stream itself is
		// perfectly well-formed.
		case 1:
			return zr
		// Predictor 12 selects the PNG "Up" filter for every row. pngUpReader
		// below only implements that one PNG filter type - real PNG
		// prediction is per-row (each row carries its own leading filter-
		// type byte: 0 None, 1 Sub, 2 Up, 3 Average, 4 Paeth), and PDF
		// Predictor values 10, 11, 13, 14 and 15 only name which filter type
		// the *encoder* used, not a promise that every row is Up. Predictor
		// 12 in particular is documented as "Up on all rows", which is
		// exactly what pngUpReader assumes and checks (it errors if a row's
		// filter-type byte isn't 2). The other PNG predictor values, and
		// TIFF predictor 2, fall through to the explicit "unsupported
		// predictor" panic below rather than being silently misdecoded or
		// lumped in with a genuinely unknown value.
		case 12:
			return &pngUpReader{r: zr, hist: make([]byte, 1+columns), tmp: make([]byte, 1+columns)}
		case 2, 10, 11, 13, 14, 15:
			panic(fmt.Sprintf("unsupported predictor %d (only Predictor 1 \"no prediction\" and Predictor 12 \"PNG Up\" are implemented)", pred.Int64()))
		}
	case "ASCII85Decode":
