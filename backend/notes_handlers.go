...
func createNoteHandler(c echo.Context) error {
	...
	// documentEnrichFlag, not a hardcoded false (ADR 0120 - which revises
	// the plan §8 default this replaced): a captured note now uses the
	// exact same readiness rule an upload already used
	// (uploadDocumentHandler, documents_handlers.go), so it enriches
	// automatically whenever Mate is ready, with no separate per-note
	// consent step. Checked before any file I/O below, so a readiness-check
	// failure fails the request outright (AGENTS.md's fallback policy) with
	...
	inserted, err := globalDocumentStore.InsertNote(document{
		...
		// ADR 0120: same readiness-based rule uploads use, computed above.
		Enrich: enrich,
	})
	if err != nil {
