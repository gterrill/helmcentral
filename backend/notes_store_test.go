...
// ── EnrichBackfillNotes (ADR 0120's automatic sweep) ─────────────────────
//
// There is no classify-candidates test here any more: classification runs
// on create and on every body edit already (notes_handlers.go), so there
// was never a real backlog for the deleted classify-backfill endpoint to
// find - see ADR 0120.
