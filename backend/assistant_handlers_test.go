...
// TestPostAssistantMessageHandler_ForcedFinalRoundDoesNotPersistBudgetInstruction
// drives a forced-final round (assistantMaxToolRounds tool-call rounds,
// followed by a plain-text answer once tools are withdrawn) through the
// real handler and the real *assistantRunner - not fakeAssistantRunner - by
// swapping newAssistantRunner to return one wired to a queuedChatDoer, the
// same way assistant_run_test.go's forced-final tests do it, but this time
// with globalAssistantStore's actual persistence on the other end. This is
// the level at which "the stored conversation doesn't contain the budget
// instruction" is actually decided: assistant_handlers.go's goroutine
// persists exactly reply.Content as the assistant row (see
// postAssistantMessageHandler's AppendMessage call), so what reaches SQLite
// is whatever run() returned, not whatever it privately said to the model
// to get there.
func TestPostAssistantMessageHandler_ForcedFinalRoundDoesNotPersistBudgetInstruction(t *testing.T) {
	secrets := withTestSecretsStore(t)
	if err := secrets.Set("OPENROUTER_API_KEY", "sk-test"); err != nil {
		t.Fatalf("Set: %v", err)
	}
	store := withTestAssistantStore(t)
	t.Setenv("SETTINGS_FILE", writeAssistantSettingsFixture(t, true, "google/gemini-3.8-flash"))
	...
	const wantAnswer = "The exhaust temperature and tank level paths both stopped updating at 14:02 local; I could not check further back than that."
	...
	for i := 0; i < assistantMaxToolRounds; i++ {
		responses = append(responses, toolCallResponse(t, fmt.Sprintf("call_%d", i), "get_last_recorded", `{"path":"tanks.0.currentLevel"}`, openRouterUsage{}))
		errs = append(errs, nil)
	}
	responses = append(responses, finalResponse(t, wantAnswer, "google/gemini-3.8-flash", openRouterUsage{}))
	errs = append(errs, nil)
	doer := &queuedChatDoer{responses: responses, errs: errs}

	swapAssistantRunner(t, func(apiKey, model, settingsPath string, autoRouter assistantAutoRouterOptions, emit assistantEmitter) assistantRunnerFace {
		return &assistantRunner{doer: doer, apiKey: apiKey, model: model, tools: &fakeToolExecutor{}, emit: emit}
	})

	c, rec := newAssistantEchoContext(http.MethodPost, "/api/assistant/conversations/"+conv.ID+"/messages",
		`{"content":"When did the exhaust temperature and tank levels stop updating?"}`, conv.ID)
	if err := postAssistantMessageHandler(c); err != nil {
		t.Fatalf("handler returned error: %v", err)
	}

	if !strings.Contains(rec.Body.String(), "event: message") {
		t.Fatalf("expected the run to finish with a message event, got:\n%s", rec.Body.String())
	}
	...
	if messages[1].Role != "assistant" || messages[1].Content != wantAnswer {
		t.Fatalf("expected the assistant row to hold exactly the model's answer, got %+v", messages[1])
	}
	for _, m := range messages {
		if strings.Contains(m.Content, assistantForcedFinalInstruction) {
			t.Fatalf("stored conversation must not contain the budget-spent instruction, got row %+v", m)
		}
	}
}
