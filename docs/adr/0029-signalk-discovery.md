...
Setting Helmcentral up required entering the SignalK server's IP in Settings → SignalK. If the address changed, telemetry stopped and the error appeared only in the settings field.
...
4. **The prompt is quiet until it has something actionable.** It searches in the background when SignalK is unconfigured or `vessel-state` reports `signalk-unreachable`, and raises a dialog **only** once at least one server is found. It does not put a modal over the dashboard to announce that it is searching, nor to report that it found nothing or could not run.
   ...
   The first implementation opened the dialog immediately with "Looking for SignalK…", blocking the dashboard throughout a multi-second sweep on every fresh start. This broke five unrelated navigation-guard tests by covering the app. Those tests passed unchanged after the dialog was limited to completed searches with results.

5. **One search per mount, and dismissal is scoped to the address.** The unreachable state persists across polls, so searching on each poll would cause repeated scans. Remembering dismissal against the affected address allows a prompt when a different server later fails.

## Consequences

Positive:
- A new install can discover its server when the search network can be derived.
- Discovery can find a server whose address has changed and offer the new address for confirmation.
...
Tradeoffs:
- **The backend port-scans the operator's own LAN.** Automatic scans are bounded, RFC1918-only, and triggered by an unconfigured or unreachable server, not a background loop. Network monitoring may detect them.
