...
Extends ADR 0039, which recorded that "gauges show instantaneous values only" and that trend for an arbitrary path needed a generic history store. It turned out not to.
...
Weighed against a per-path in-memory store — a map of ring buffers, a memory budget, a decision about which paths to record, and still only hours of history — the existing dependency does the job better for the cost of nothing.
...
This is not defensive politeness, it is the point. **A flat line drawn because no database exists is indistinguishable from a sensor reading a steady value.** It is the trend-shaped version of the failure ADR 0039's structural dash exists to prevent, and AGENTS.md's fallback policy already forbids masking a missing upstream source. The tile renders the server's own message — "history needs InfluxDB, which is not configured" — and keeps showing the live reading, which does not depend on it.
...
The trend scale reuses the gauge's own `min`/`max` when set, so a dial and a trend bound to the same path agree rather than telling two stories.
...
`go test -short ./...` and the frontend suite pass. The backend tests cover the 503 with a reason, the 400s for a missing path and an unknown window, and the injection attempt through `window`. The frontend tests cover a rendered line, and — the one that matters — that a 503 renders the message rather than an empty chart.
