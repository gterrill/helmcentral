...
`newWasmPluginBase` (`backend/wasm_plugin.go`) handles optional guest exports by calling them if present, logging failures, and otherwise using a default. `description()` follows the existing `ttl_seconds()` pattern: it is called if `instance.FunctionExists("description")`; a call failure is logged and returns `""`; absence returns `""` without a log. Unlike `id()`/`name()`, a missing or failing `description()` does not fail `newWasmPluginBase`, because it is optional display metadata.

`wasmPluginBase` gained a `description string` field and `Description() string` accessor. All four WASM provider types (`wasmTideProvider`, `wasmWeatherProvider`, `wasmWaveProvider`, `wasmForecastWarningsProvider`) embed `*wasmPluginBase` and inherit the accessor through Go's method promotion. `Description() string` was added to the four provider interfaces (`tideProvider`, `weatherProvider`, `waveProvider`, `forecastWarningsProvider`); the native `stormGlassTideProvider` received a hand-written implementation. The four `*ProviderInfo` response structs gained a backward-compatible `Description string` field, populated by their `*ProvidersHandler` alongside `ID()`/`Name()`.
...
- **No encryption need.** Overrides contain hostnames and secret key names (e.g. `"WEATHERKIT_KEY_ID"`), not secret values. Encrypting them was not required and would couple the store to `secretsStore`'s master-key resolution and integrity checks.
...
Tradeoffs:
- No hot-reload: every override save requires an operator-initiated restart to take effect. The frontend must communicate that requirement.
