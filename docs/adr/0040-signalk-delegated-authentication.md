...
`HELMCENTRAL_AUTH_MODE` is a documented environment-variable override that wins over `settings.yaml` in both directions — the recovery hatch for an operator locked out of `mode: signalk` (a broken SignalK security config, a forgotten password), since the Settings page that would normally flip this setting back sits behind `admin`, which is exactly the tier a locked-out operator can't reach.
