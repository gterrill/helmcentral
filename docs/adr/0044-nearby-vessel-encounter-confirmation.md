...
- `summary`'s `seen_count` undercounts by one for up to `contactConfirmDwell` while a returning vessel's candidacy is pending. It corrects itself on confirmation and is not otherwise adjusted here (see `summary`'s doc comment).
