...
Reusing ADR-0017's WASM plugin machinery (`backend/wasm_tide_provider.go`, Extism/wazero) required answering two questions:
1. Could the ~500-line adapter be generalized instead of copied for the third and fourth provider types?
2. Could TinyGo targeting `wasip1` use `crypto/ecdsa`/`crypto/x509` to create WeatherKit's ES256-signed JWT from a PEM private key inside the guest, or would this require a new host-side signing function across the sandbox boundary?
...
Shared code from `wasm_tide_provider.go` was extracted into `backend/wasm_plugin.go`: `wasmPluginBase` (id/name/ttl + compiled module + `call()`), `wasmPluginCache[T]` (a generic TTL/stale/disk-persisted cache replacing `wasmTideCache`), `manifestForWasmPlugin`/`configForWasmPlugin`/`allowedHostsForWasmPlugin` (companion-file loaders), `newWasmPluginBase` (compilation and validation of `id()`/`name()`/`ttl_seconds()`), and `loadWasmPluginsFromDir` (discovery and registration). `wasm_tide_provider.go` was refactored onto this base in the same change. `wasm_weather_provider.go` and `wasm_wave_provider.go` are each under 150 lines, containing a typed cache and one contract-specific fetch method. Future provider types can reuse allowlisting, timeouts, panic containment, and caching.
...
`condition` uses the existing WeatherKit condition-code vocabulary (`clear`, `cloudy`, `mostlycloudy`, `partlycloudy`, `rain`, `heavyrain`, `snow`, `thunderstorms`, `mixedrainandsnow`, etc.). The full list is documented in `backend/weather_providers.go` and enforced by `formatWeatherConditionAt` in `main.go`. WeatherKit's `conditionCode` values already match, and the display logic, including the `mostlyclear` day/night split, remains unchanged. Open-Meteo maps WMO weather codes onto this vocabulary and falls back to `"cloudy"` for unmapped codes. That fallback is an accepted reference-plugin simplification; the mapping is not exhaustive.
...
- A future plugin type can reuse `wasm_plugin.go` for allowlisting, timeouts, panic containment, and caching.
- Apple WeatherKit remains supported through a plugin, with tested credential-acquisition instructions.

Negative / explicitly deferred:
- The tide plugin disk-cache format change (see "Shared host layer" above) discards one cached tide-chart fetch on first boot after upgrading. The next fetch rebuilds it.
