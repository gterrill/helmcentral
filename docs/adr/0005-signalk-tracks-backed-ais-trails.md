...
The motoring approach trail and the self post-anchor ring-buffer remain on the existing Helmcentral-owned paths. They are live-sampled per ADR-0020, which removed Influx startup seeding, and serve anchor reposition context rather than AIS history.

## Consequences
Positive:
- AIS trail history survives server restarts; the plugin retains history across Helmcentral restarts.
- The SignalK plugin owns AIS history, replacing two parallel accumulators.
...
- The `since` incremental-update contract is no longer applied to AIS trails: every poll returns the full retained snapshot from the plugin. The frontend handles this by replacing rather than appending, and the plugin's sliding-window retention determines the history window.
