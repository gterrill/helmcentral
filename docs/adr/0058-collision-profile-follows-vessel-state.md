...
> **Amended 2026-09-11 by ADR 0090.** The same analysis, run alongside a marina pontoon on the shipped `harbor` profile, found the warning tier's 0.5 knot speed floor tripping on GPS jitter from tied-up neighbours every few minutes: 100 raises in four and a half hours, nearly all `warn` on boats doing nothing. In a marina every target is inside the 0.5 NM warning CPA and, for two stationary boats, CPA equals range, so the speed floor is the only condition doing any work. A starting point from that analysis: warning tier CPA 0.1 NM, TCPA 5 minutes, speed 2 knots; danger tier left at the shipped 3 knots, which already excluded every stationary neighbour and caught the one boat actually moving down the fairway. ADR 0090 puts the profile and tier in force on the alarm card so the next such night can be read off the drawer rather than out of the plugin's source.
