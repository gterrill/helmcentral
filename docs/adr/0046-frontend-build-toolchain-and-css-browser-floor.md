...
## Addendum (2026-09-17): the wall's browser, checked before Tailwind v4

Tailwind v4's own generated CSS needs more than this ADR's `cssTarget: safari16.4` promises to downlevel for. Cascade layers (`@layer`), registered custom properties (`@property`) and `color-mix()` are all load-bearing in what v4 ships, not just a compile target `lightningcss` can flatten away, and all three landed in Safari 16.4 (March 2023). The entry-chunk addendum above already set a stricter, older JS-parse floor for the wall specifically (WPE WebKit 2.38.5, Safari 16.0), so before touching the CSS toolchain at all, the open question was whether the wall had actually moved past that.

It has. The operator confirmed the ODROID kiosk now runs `wpe-webkit-mir-kiosk` 2.44.1 (the 22/candidate snap channel), reporting a Safari 17.0-era user agent, past both this ADR's `cssTarget` floor and the entry chunk's older one. `frontend/public/kiosk-probe.html` checks the three v4-specific requirements directly (`@layer` behaviourally, since `CSS.supports` can't test an at-rule; `@property`'s initial-value application; `color-mix()` via `CSS.supports`) alongside the existing `:has()` and lookbehind checks, so the next browser regression on this device gets caught by the probe instead of a blank screen.

Nothing here changes either floor this ADR and its addendum already set. The lazy-loaded-code floor stays Baseline 2024 (Safari 16.4); the entry chunk's floor stays Safari 16.0 for JS-parse purposes. What's new is confirmation that the one device that has actually forced this ADR's hand before (the kiosk wall) currently clears both.